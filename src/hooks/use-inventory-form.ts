...
export const useInventoryForm = () => {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      // Build final product name: if user provided a custom name, append brand ("<name> - <brand>")
      const selectedBrand = brands.find((b) => b._id === formData.brand);
      const brandName = selectedBrand?.name || "Unknown Brand";
      let finalName = generateProductName();
      const custom = formData.productName?.trim();
      if (custom) {
        const lowerCustom = custom.toLowerCase();
        const lowerBrand = brandName.toLowerCase();
        // If brand already present in custom name, keep as-is; else append
        finalName = lowerCustom.includes(lowerBrand)
          ? custom
          : `${custom} - ${brandName}`;
      }
      ...
      if (result.success) {
        // Ensure UI has hydrated products (brand/category populated)
        await fetchProducts();
