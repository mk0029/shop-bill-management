...
export const useInventoryStore = create<InventoryStore>((set, get) => ({
  ...
  handleRealtimeProductCreated: async (product: Product) => {
    const { products } = get();
    const existingProduct = products.find((p) => p._id === product._id);

    // If we don't already have it, hydrate to ensure brand/category are populated
    if (!existingProduct) {
      try {
        const fullRes = await inventoryApi.getProductById(product._id);
        const hydrated = fullRes.success && fullRes.data ? (fullRes.data as Product) : product;
        set((state) => ({ products: [hydrated, ...state.products] }));
      } catch {
        // Fallback to raw product if hydration fails
