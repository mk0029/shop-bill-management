...
export default function ShopStatusMessagesSettings() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/shop-status-messages", { cache: "no-store" });
  ...
  const updateField = (key: ShopStatusKey, field: "title" | "body", value: string) => {
    setMessages((current) => ({
      ...current,
      [key]: {
        ...current[key],
  ...
  const save = async () => {
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.success) throw new Error(json?.error || "Failed to save messages");
      setMessages(json.data);
      toast.success("Shop status messages saved");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to save messages");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-950 px-4 py-5 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl space-y-5">
        <div className="flex flex-col gap-3 border-b border-slate-800 pb-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-normal">Shop Status Messages</h1>
        ...
        <div className="grid gap-4">
          {rows.map((row) => (
            <section key={row.key} className="rounded-lg border border-slate-800 bg-slate-900/60 p-4">
              <div className="mb-3">
              ...
              <div className="grid gap-3">
                <label className="grid gap-1.5">
                  <span className="text-xs font-medium text-slate-300">Notification title</span>
                  <input
                    value={messages[row.key].title}
                    onChange={(event) => updateField(row.key, "title", event.target.value)}
                ...
                <label className="grid gap-1.5">
                  <span className="text-xs font-medium text-slate-300">Notification message</span>
                  <textarea
                    value={messages[row.key].body}
                    onChange={(event) => updateField(row.key, "body", event.target.value)}
