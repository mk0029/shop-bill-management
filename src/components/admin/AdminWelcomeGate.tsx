...
const SESSION_PREFIX = "admin_welcome_seen";
...
function seenKey(userId?: string) {
  return `${SESSION_PREFIX}:${userId || "staff"}`;
}
...
function StaffBackground() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#050914]">
      ...
      <motion.div
        className="absolute left-0 top-[36%] h-px w-full bg-gradient-to-r from-transparent via-orange-200/70 to-transparent"
        animate={
          reducedMotion
            ? { opacity: 0.35 }
            : { x: ["-42%", "42%"], opacity: [0, 0.75, 0] }
        }
        transition={{
          duration: 4.2,
          repeat: reducedMotion ? 0 : Infinity,
...
export default function AdminWelcomeGate() {
  ...
  return (
    <motion.section
      className="fixed inset-0 z-[90] overflow-y-auto bg-slate-950"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
