...
export const BottomActionBar = memo(function BottomActionBar({
  ...
  return (
    ...
    >
      <div className=" p-3 sm:px-5 flex-1 bg-slate-950/90 rounded-lg md:rounded-2xl">
        <div className="flex items-center justify-evenly gap-1.5 sm:gap-2.5 overflow-x-visible no-scrollbar">
          {primaryActions.map((action) => (
            <button
              key={action.label}
              onClick={action.onClick}
              className={cn(
                "flex flex-col items-center gap-1 min-w-[60px] sm:min-w-[80px] px-1 sm:px-3 py-2 sm:py-2.5 rounded-2xl text-[11px] sm:text-xs font-medium whitespace-nowrap transition-all duration-200 shrink-0",
