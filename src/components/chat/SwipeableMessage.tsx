...
export function SwipeableMessage({
  ...
  const renderMessage = () => (
    <div className={`flex ${isSelf ? "justify-end" : "justify-start"}`}>
      <div
        className="relative max-w-[75%] w-full overflow-hidden"
        ref={constraintsRef}
      >
        <motion.div
          drag="x"
        ...
        >
            ...
            className={`text-sm px-2.5 py-1.5 border shadow-md leading-[120%] ${
              ...
              groupPosition === "single"
                ...
                : groupPosition === "first"
                  ? isSelf
                    ? // First in group (self)
                      "rounded-2xl rounded-br-md rounded-tr-2xl"
                    : // First in group (other)
                      "rounded-2xl rounded-bl-md rounded-tl-2xl"
                  : groupPosition === "middle"
                    ? isSelf
                      ? // Middle in group (self) — tight stack
                        "rounded-lg rounded-r-md"
                      : // Middle in group (other)
                        "rounded-lg rounded-l-md"
                    : // last
                      isSelf
                      ? // Last in group (self)
                        "rounded-2xl rounded-tr-md rounded-br-2xl"
                      : // Last in group (other)
          ...
          >
            ...
            <div className="whitespace-pre-wrap leading-[120%]">
              {message.content}
            </div>
                  ...
                  return (
                    <>
                      ...
                      {images.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {images.map((attachment, idx) => {
                            ...
                            const isUploading =
                              attachment._id &&
                              uploadProgress[attachment._id] !== undefined &&
                              uploadProgress[attachment._id] < 100;
                            const progress = attachment._id
                              ? uploadProgress[attachment._id] || 0
                              : 0;
                            ...
                            if (images.length === 1) {
                              ...
                              const isLastRow =
                                idx >= Math.floor(images.length / 3) * 3;
                              const remainingInLastRow = images.length % 3;

                              if (
                                isLastRow &&
                                remainingInLastRow === 1 &&
                                idx === images.length - 1
                              ) {
                                flexBasis = "100%";
                                maxWidth = "100%";
                            ...
                            return (
                              <div
                                ...
                                className={`relative rounded-lg overflow-hidden border p-1 ${idx === 0 && images.length === 1 && "!min-w-[150px]"} ${
                                  isSelf
                                    ? "border-slate-500/70 bg-white/10 backdrop-blur-sm"
                                    : "border-zinc-300 dark:border-zinc-500/50"
                                }`}
                                style={{
                                  flexBasis,
                                  maxWidth,
                                  aspectRatio:
                                    images.length === 1 ? "auto" : "1/1",
                              ...
                              >
                                ...
                                {attachment.url && !isUploading && (
                                  <button
                                    onClick={() =>
                                      openPreview(attachment as ChatAttachment)
                                    }
                      ...
                      {nonImages.map((attachment, idx) => {
                        ...
                        const isAudio =
                          attachment.type?.startsWith("audio/") || false;
                        ...
                        return (
                          <div
                            key={idx}
                            className={`border rounded-lg p-2 md:p-3 relative ${
                              isSelf
                          ...
                          >
                            ...
                            {!attachment.url && isUploading && (
                              <div
                                style={{
                                  width: "100%",
                                  height: `${Math.max(50, Math.min(200, attachment.size / 1000))}px`,
                            ...
                            {isAudio && (
                              <div>
                                {attachment.url ? (
                                  ...
                                  <div
                                    className={`flex items-center gap-3 min-w-[220px] p-3 rounded-lg ${
                                      isSelf
                                        ? "bg-white/10"
                                  ...
                                  >
                                    ...
                                    <div
                                      className={`text-sm ${isSelf ? "text-white" : "text-zinc-600 dark:text-zinc-400"}`}
                                    >
                                      {isUploading
            ...
            >
              <span
                className={`text-[11px] !leading-none ${isSelf ? "text-white/80" : "opacity-70"}`}
              >
                ...
                {showSenderName && senderName && (
                  <span className="ml-1 font-medium">• {senderName}</span>
                )}
              </span>
              {message.editedAt && (
                <span
                  className={`text-[10px] italic ml-auto ${isSelf ? "text-white/70" : "opacity-60"}`}
                >
                  edited
                </span>
              )}
