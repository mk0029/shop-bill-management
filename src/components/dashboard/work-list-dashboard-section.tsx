...
type CreateState = {
  ...
  issueCategory: "repair" | "fitting" | "wiring" | "delivery" | "payment" | "other";
  dueAt: string;
};
...
export default function WorkListDashboardSection({ users }: { users: UserLite[] }) {
  ...
  const technicians = useMemo(
    () => users.filter((u) => ["admin", "super_admin", "technician"].includes(String(u.role || "").toLowerCase())),
    [users],
  );
  ...
  const load = useCallback(async (opts?: { silent?: boolean }) => {
    try {
      ...
      const deduped = Array.from(new Map((data || []).map((t) => [t._id, t])).values());
      setTasks(deduped);
    } catch (e) {
  ...
  const createTask = async () => {
    ...
    if (!createState.assignedTechnicianId) return toast.error("Assignee is required");
    ...
    try {
      ...
      const optimisticTask = {
        ...
        customerRef: customers.find((c) => c._id === payload.customerRefId) || null,
        assignedTechnician: technicians.find((t) => t._id === payload.assignedTechnicianId) || null,
        assignedTechnicianName:
          technicians.find((t) => t._id === payload.assignedTechnicianId)?.name || "",
      ...
      setTasks((prev) => {
        ...
        return Array.from(new Map([created, ...next].map((t) => [t._id, t])).values());
      });
  ...
  const markComplete = async () => {
    ...
    try {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t._id === activeTask._id ? { ...t, status: "completed", completedAt: nowIso, updatedAt: nowIso } : t,
        ),
      );
      await workTaskService.updateWorkTask(activeTask._id, { status: "completed" });
  ...
  return (
    <Card className="bg-gray-900 border-gray-800">
      <CardContent className="p-3 sm:p-4">
        <ResponsiveAccordion title="Work List" defaultOpenOnDesktop defaultExpandedOnMobile={false}>
              ...
              <div className="space-y-2">
                {pendingTasks.slice(0, 8).map((task) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-400">
                      {task.assignedTechnicianName || task.assignedTechnician?.name || "-"} • {task.priority} • {task.status}
                    </p>
                    <p className="text-xs text-gray-500">Due: {formatDayDateTime(task.dueAt)}</p>
                    {task.description ? (
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Work" size="md">
        <div className="space-y-3">
          ...
          <div>
            <p className="text-xs text-gray-300 mb-1">Description / Issue Notes</p>
            <Input
          ...
          <div>
            <p className="text-xs text-gray-300 mb-1">Customer Reference (Optional)</p>
            <CustomerAutocomplete
              customers={customers}
              value={createState.customerRefId}
              onChange={(id) => setCreateState((p) => ({ ...p, customerRefId: id }))}
          ...
          <SelectField
            ...
            onChange={(v) => setCreateState((p) => ({ ...p, assignedTechnicianId: v }))}
            options={technicians.map((t) => ({ value: t._id, label: `${t.name} (${t.role})` }))}
          ...
          <div className="grid grid-cols-2 gap-2">
            <SelectField
              ...
              onChange={(v) => setCreateState((p) => ({ ...p, priority: v as any }))}
            ...
            <SelectField
              ...
              onChange={(v) => setCreateState((p) => ({ ...p, issueCategory: v as any }))}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={createTask} disabled={saving}>{saving ? "Saving..." : "Save Work"}</Button>
          </div>
        </div>
      </Modal>

      <Modal isOpen={!!activeTask} onClose={() => setActiveTask(null)} title="Work Details" size="sm">
        {activeTask ? (
          <div className="space-y-3">
            <p className="text-white font-medium">{activeTask.title}</p>
            <p className="text-xs text-gray-400">
              Assigned: {activeTask.assignedTechnicianName || activeTask.assignedTechnician?.name || "-"}
            </p>
            <p className="text-xs text-gray-400">
              Status: {activeTask.status} • Priority: {activeTask.priority}
            </p>
            <p className="text-xs text-gray-500">Due: {formatDayDateTime(activeTask.dueAt)}</p>
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={activeTask.status === "completed" || activeTask.status === "cancelled"}
                className="flex-1"
              >
                Mark Complete
              </Button>
