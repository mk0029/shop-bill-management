...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.ocrText) {
      return NextResponse.json(
        { success: false, error: "OCR text is required. Tesseract.js must extract text in the browser first." },
        { status: 400 }
      );
    }

    const sizeEstimate = Math.round((body.image.length * 3) / 4);
    ...
    const response = await fetch(`${BACKEND_URL}/api/payment-verification/verify-receipt`, {
      method: "POST",
      headers: {
    ...
    if (!contentType.includes("application/json")) {
      ...
      const snippet = text.replace(/^<[^>]*>/, "").substring(0, 100).trim();
      return NextResponse.json(
        {
          success: false,
          error: `Backend returned non-JSON response (HTTP ${response.status}). The verification server may be unavailable. ${snippet ? `Response: "${snippet}"` : ""}`,
        },
        { status: 502 }
      );
    }
