...
function getStockStatusIcon(status: StockStatus | string) {
  const statusObj = typeof status === 'string' ? { status: status as StockStatus['status'], label: status } : status;
...
export default function InventoryPage() {
  ...
  const filteredItems = (products || []).filter((product) => {
    ...
    const matchesCategory = categoryFilter === "all" || 
      (product.category?._id === categoryFilter || categoryName.toLowerCase() === categoryFilter.toLowerCase());

    const stockStatus = getProductStockStatus(product);
    const matchesStockStatus = 
      stockStatusFilter === "all" || 
      stockStatus.status === stockStatusFilter;
                  ...
                  <div className="relative">
                    ...
                    {showFilters && (
                      <div className="absolute right-0 z-20 mt-2 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                        <div className="py-1">
                          <DropdownMenuItem onClick={() => { setCategoryFilter("all"); setShowFilters(false); }}>
                            All Categories
                          </DropdownMenuItem>
                          {categoryOptions.map((category) => (
                            <DropdownMenuItem 
                              key={category.value}
                              onClick={() => { setCategoryFilter(category.value); setShowFilters(false); }}
                            >
                              {category.label}
                            </DropdownMenuItem>
                          ))}
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => { setStockStatusFilter("all"); setShowFilters(false); }}>
                            All Stock Status
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => { setStockStatusFilter("in-stock"); setShowFilters(false); }}>
                            In Stock
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => { setStockStatusFilter("low-stock"); setShowFilters(false); }}>
                            Low Stock
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => { setStockStatusFilter("out-of-stock"); setShowFilters(false); }}>
                            Out of Stock
                          </DropdownMenuItem>
                        </div>
                      </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredItems.map((product) => {
                      ...
                      const itemValue = (product.inventory?.currentStock || 0) * (product.pricing?.purchasePrice || 0);
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={getStockStatusColor(status)}>
                                {product.inventory?.currentStock || 0} {product.pricing?.unit || ''}
                              </span>
                            </div>
                          </td>
                          <td className="py-4 px-4">
                            {currency} {product.pricing?.purchasePrice?.toFixed(2) || '0.00'}
                          </td>
                          <td className="py-4 px-4">
                            {currency} {product.pricing?.sellingPrice?.toFixed(2) || '0.00'}
                          </td>
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => router.push(`/admin/inventory/edit/${product._id}`)}
                              >
        ...
        <Dialog open={!!deleteItem} onOpenChange={(open: boolean) => !open && setDeleteItem(null)}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <p>Are you sure you want to delete &quot;{deleteItem?.name}&quot;? This action cannot be undone.</p>
              {deleteError && (
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {typeof selectedItem.brand === 'string' ? selectedItem.brand : selectedItem.brand?.name}
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      getStockStatusColor(getProductStockStatus(selectedItem).status)
                    }`}>
                      {getProductStockStatus(selectedItem).label}
                    </span>
                    <span className="text-sm font-medium">
                      {currency}{selectedItem.pricing?.sellingPrice?.toFixed(2) || '0.00'}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-white">{typeof selectedItem.category === 'string' ? selectedItem.category : selectedItem.category?.name}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Product ID</p>
                    <p className="text-white">{selectedItem.productId || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Quantity</p>
                    <p className="text-white">{selectedItem.inventory?.currentStock || 0} {selectedItem.pricing?.unit || 'units'}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Low Stock Threshold</p>
                    <p className="text-white">{selectedItem.inventory?.minimumStock || 'Not set'}</p>
                  </div>
                </div>
                    ...
                    <div className="mt-2 space-y-1">
                      {Object.entries(
                        ...
                        <div key={key} className="flex text-sm">
                          <span className="font-medium text-gray-500 w-32 flex-shrink-0">{key}:</span>
