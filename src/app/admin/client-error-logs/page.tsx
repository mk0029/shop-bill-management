...
export default function ClientErrorLogsPage() {
  ...
  const loadLogs = async () => {
    ...
    try {
      const response = await fetch(
        `/api/client-error-logs?limit=75${oppoOnly ? "&oppo=1" : ""}`,
      );
  ...
  const title = useMemo(
    () => (oppoOnly ? "OPPO Client Errors" : "Client Errors"),
    [oppoOnly],
  );

  return (
    <div className="h-[var(--app-vh,100dvh)] bg-gray-950 p-3 text-white sm:p-5">
      <div className="mx-auto max-w-6xl space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                Runtime crashes, browser API failures, and unhandled promises.
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={() => setOppoOnly((value) => !value)}
            >
            ...
            <CardHeader className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <CardTitle className="text-base text-white">
                  {log.message || "Unknown client error"}
                </CardTitle>
                {log.isOppo ? (
                  <Badge className="bg-amber-500 text-gray-950">OPPO</Badge>
                ) : null}
              ...
              <div className="text-xs text-gray-400">
                {log.receivedAt || log.occurredAt || "Unknown time"}{" "}
                {log.route ? `- ${log.route}` : ""}
              </div>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              <div className="grid gap-2 md:grid-cols-2">
                ...
                <div className="rounded-md bg-gray-950 p-3">
                  ...
                  <div className="break-all text-gray-200">
                    {log.browser || log.platform || "Unknown"}
                  </div>
                </div>
              </div>
              {log.stack ? (
              ...
              {log.userAgent ? (
                <div className="break-all rounded-md bg-gray-950 p-3 text-xs text-gray-400">
                  {log.userAgent}
                </div>
              ) : null}
            </CardContent>
