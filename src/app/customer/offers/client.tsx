...
export function CustomerOffersClient() {
  ...
  useEffect(() => {
    const sub = sanityClient
      ...
      .subscribe({
        ...
        error: (error) =>
          console.warn("[offers] realtime refresh failed", error),
      });
  ...
  const getStatus = (offer: OfferWithProduct) =>
    getComputedOfferStatus(offer.startAt, offer.endAt);
    ...
    <div className="space-y-6 p-4 sm:p-6 h-full">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          Grab exclusive deals on your favorite products
        </p>
      </div>
      ...
      {loading ? (
        ...
        >
          ...
          <p className="mt-2 max-w-sm text-center text-sm text-gray-400">
            There are no active offers right now. We&apos;ll notify you when a
            ...
            return (
              ...
              >
                {upcoming ? (
                  <>
                    {/* Scheduled teaser */}
                    <div className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1 text-[10px] font-medium text-amber-300">
                      <Timer className="h-3 w-3" />
                      Coming Soon
                    </div>
                    ...
                    <div className="mt-4">
                      <Button
                        disabled
                        variant="outline"
                        className="w-full gap-2 border-amber-500/20 text-amber-400"
                      >
                  ...
                  <>
                    ...
                    <div className="mb-4 flex items-center gap-3">
                      <div
                        className={`rounded-xl p-3 ring-1 ring-white/[0.06] ${
                          expired
                            ? "bg-gray-500/10"
                      ...
                      >
                        <DiscountIcon
                          className={`h-6 w-6 ${expired ? "text-gray-500" : "text-cyan-400"}`}
                        />
                      </div>
                      <div>
                        <p
                    ...
                    {/* Title */}
                    <h3
                      className={`text-sm font-semibold ${expired ? "text-gray-500" : "text-white"}`}
                    >
                      {offer.title}
                    </h3>
                    {offer.description && (
        ...
        {missedOffer && (
          <div className="flex flex-col items-center px-4 py-6 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400">
              ...
              offer. It has expired.
            </p>
            <p className="mt-1 text-sm text-gray-500">
              Our new offers are on the way. Stay tuned!
            </p>
            <div className="mt-6 flex items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-4 py-2 text-xs text-cyan-300">
              <Sparkles className="h-3.5 w-3.5" />
          ...
          <>
            ...
            <div className="mt-3 border-t border-white/[0.06] px-2 pt-3">
              <p className="text-center text-[11px] text-gray-500">
                {(() => {
                  const s = getComputedOfferStatus(
                    productListOffer.startAt,
                    productListOffer.endAt,
                  );
                  switch (s) {
                    case "scheduled":
                      return "Offer will be available on";
                    case "expired":
                      return "Offer was available on";
                    default:
                      return "Offer is available on";
                  }
                })()}{" "}
