...
function CustomerBillStats({ customerBills = [] }: { customerBills: Bill[] }) {
  const stats = useMemo(() => {
    ...
    const paidBills = customerBills.filter(bill => bill.paymentStatus === "paid");
    const pendingBills = customerBills.filter(bill => bill.paymentStatus === "pending");
    const overdueBills = customerBills.filter(bill => bill.paymentStatus === "overdue");
    const partialBills = customerBills.filter(bill => bill.paymentStatus === "partial");
    ...
    const partialPaidAmount = partialBills.reduce((sum, bill) => sum + (bill.paidAmount || 0), 0);
    const pendingAmount = pendingBills.reduce((sum, bill) => sum + (bill.totalAmount || 0), 0);
    const partialPendingAmount = partialBills.reduce((sum, bill) => sum + (bill.balanceAmount || 0), 0);
  ...
  if (customerBills.length === 0) {
    return (
      <div className="text-center py-12 border-2 border-dashed border-gray-800 rounded-lg">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-300">No bills found</h3>
...
function CustomerHomeContent() {
  ...
  const { customer, loading: customerLoading, error: customerError, refresh } = useCustomerData();
  const { bills, loading: billsLoading, fetchBillsByCustomer } = useSanityBillStore();
  const router = useRouter();
  ...
  }, [customer?._id, fetchBillsByCustomer]);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Welcome back, {customer.name}!</h1>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <Button className="w-full justify-start" asChild>
                  <Link href="/customer/bills/new" className="flex items-center">
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-400">Customer ID</p>
                    <p className="text-sm text-white">{customer.customerId || 'N/A'}</p>
                  </div>
                </div>
                <div className="flex items-start space-x-4">
                  <div className="p-2 rounded-lg bg-blue-500/10 text-blue-400 flex-shrink-0">
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white">{customer.location || 'N/A'}</p>
                  </div>
                </div>
              </div>
            </CardContent>
                  ...
                  <div key={bill._id} className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        {formatDate(bill.billDate || bill._createdAt)} • {formatCurrency(bill.totalAmount || 0)}
                      </p>
                    </div>
